...
"""
    ...
    file_name = f"{entry['ID']}_{title.replace(' ', '_').lower()}.html"
...
def generate_master_navigation(entries):
    ...
    for master_entry in master_pages:
        ...
        master_id = master_entry['ID']
        nav_links.append(f'<li><a href="{master_id}_{master_title.replace(" ", "_").lower()}.html">{master_title}</a></li>')
        
        # Add subpage links for this master page
        sub_nav_links = [
            f'<li style="margin-left:20px;"><a href="{entry["ID"]}_{entry["TITLE"].replace(" ", "_").lower()}.html">{entry["TITLE"]}</a></li>'
