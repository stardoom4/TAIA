...
def generate_html_from_taia(file_path, output_dir):
    ...
    generate_index_page(entries, output_dir)
...
def generate_index_page(entries, output_dir):
    # Extract only master pages (pages without an 'UNDER' field)
    master_pages = [entry for entry in entries if 'UNDER' not in entry]
...
<body>
<h1>Welcome to the Site Index</h1>
<ul>
"""
    # Add each master page as a link
    for master_entry in master_pages:
        master_title = master_entry['TITLE']
        master_file_name = f"{master_title.replace(' ', '_').lower()}.html"
        index_content += f'<li><a href="{master_file_name}">{master_title}</a></li>\n'
